...
from kron.protocols import Deserializable, Serializable, implements
...
from kron.types.db_types import VectorMeta, extract_kron_db_meta
from kron.utils import compute_hash, json_dump, now_utc
...
class Node(Element):
    ...
    def update_integrity_hash(self, previous_hash: str | None = None) -> str | None:
        ...
        """
        from kron.utils import compute_chain_hash
...
def create_node(
    ...
    **config_kwargs: Any,
    ...
    """
    # Import specs lazily to avoid circular imports
    from kron.specs.catalog import AuditSpecs, ContentSpecs
    ...
    node_config = NodeConfig(
        table_name=table_name or Unset,
        schema=schema,
...
def generate_ddl(
    ...
    *,
    ...
    if config.flatten_content and content_type is not None:
        from kron.specs.adapters.pydantic_adapter import PydanticSpecAdapter
    ...
    # Use config.meta_key (default: "node_metadata") to match to_dict(mode="db")
